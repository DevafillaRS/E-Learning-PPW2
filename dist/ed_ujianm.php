...
$sql = "SELECT * FROM umahasiswa WHERE id=$id";
$query = mysqli_query($koneksi, $sql);

if ($query)
    $umahasiswa = mysqli_fetch_assoc($query);
...
if (isset($_POST['submit'])) {
    ...
    if (true) {
        ...
        $sqlUpdate = "UPDATE umahasiswa SET mahasiswa='$mahasiswa', kelas='kelas', judul='$judul', tanggal='$tanggal' WHERE id=$id";

        $queryUpdate = mysqli_query($koneksi, $sqlUpdate);
        if ($queryUpdate) {
            header('location: ed_ujianm.php?id=' . $id);
        } else {
            echo "gagal update";
                                        ...
                                        <div class="col-lg-6 col-md-12">
                                            <form action="" method="POST" enctype="multipart/form-data">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" class="form-control" name="mahasiswa" value="<?= $umahasiswa['mahasiswa'] ?>">
                                                </div>
                                                <div class="form-group">
                                                    <label>Kelas</label>
                                                    <input type="text" class="form-control" name="kelas" value="<?= $umahasiswa['kelas'] ?>">
                                                </div>
                                                <div class="form-group">
                                                    <label>Judul</label>
                                                    <input type="text" class="form-control" name="judul" value="<?= $umahasiswa['judul'] ?>">
                                                </div>
                                                <div class="form-group">
                                                    <label>Kumpulkan</label>
                                                    <input type="date" class="form-control" name="tanggal" value="<?= $umahasiswa['tanggal'] ?>">
                                                </div>
                                                <button type="submit" name="submit" class="btn btn-success">Simpan Edit</button>
                                            </form>
                                        </div>
                                    </div>
