...
if (isset($_POST['submit'])) {
    ...
    $path = "Ujian Mahasiswa/";
    $filePath = $path . $fileName;
    $result = move_uploaded_file($tmpName, $filePath);

    if ($result) {

        $sql = "INSERT INTO umahasiswa (mahasiswa, kelas, judul, fujian, tanggal) 
            VALUE ('$mahasiswa', '$kelas', '$judul', '$fileName', '$tanggal')";

        $query = mysqli_query($koneksi, $sql);
        if ($query) {
            header('location: ku_ujian.php');
        } else {
            echo "gagal submit";
                                    ...
                                    <div class="row">
                                        <div class="col-lg-6 col-md-12">
                                            <form action="" method="POST" enctype="multipart/form-data">
                                                ...
                                                <button type="submit" name="submit" class="btn btn-warning">Upload Ujian</button>
                                            </form>
                                        </div>
                                    </div>
