...
$sql = "SELECT * FROM tmahasiswa WHERE id=$id";
$query = mysqli_query($koneksi, $sql);

if ($query)
    $tmahasiswa = mysqli_fetch_assoc($query);
...
if (isset($_POST['submit'])) {
    ...
    if (true) {
        ...
        $sqlUpdate = "UPDATE tmahasiswa SET mahasiswa='$mahasiswa', kelas='kelas', judul='$judul', tanggal='$tanggal' WHERE id=$id";

        $queryUpdate = mysqli_query($koneksi, $sqlUpdate);
        if ($queryUpdate) {
            header('location: ed_tugasm.php?id=' . $id);
        } else {
            echo "gagal update";
                                        ...
                                        <div class="col-lg-6 col-md-12">
                                            <form action="" method="POST" enctype="multipart/form-data">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" class="form-control" name="mahasiswa" value="<?= $tmahasiswa['mahasiswa'] ?>">
                                                </div>
                                                <div class="form-group">
                                                    <label>Kelas</label>
                                                    <input type="text" class="form-control" name="kelas" value="<?= $tmahasiswa['kelas'] ?>">
                                                </div>
                                                <div class="form-group">
                                                    <label>Judul</label>
                                                    <input type="text" class="form-control" name="judul" value="<?= $tmahasiswa['judul'] ?>">
                                                </div>
                                                <div class="form-group">
                                                    <label>Kumpulkan</label>
                                                    <input type="date" class="form-control" name="tanggal" value="<?= $tmahasiswa['tanggal'] ?>">
                                                </div>
